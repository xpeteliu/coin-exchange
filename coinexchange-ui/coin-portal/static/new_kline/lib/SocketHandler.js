...
SocketHandler.prototype = {
  ...
  // 打开socket连接
  connect(heartbeatTimeout = 10000, callback) {
    // this.open(heartbeatTimeout)
    // this.message()
    // this.close()

    // this.stompClient.heartbeat.outgoing = heartbeatTimeout
    // this.stompClient.heartbeat.incoming = heartbeatTimeout
  ...
  on(id, callback) {
    ...
    this.stompClient.subscribe('/topic/' + id, (msg) => callback(JSON.parse(msg.body)))
  },
  ...
  unsubscribe(ch, id, token) {
    ...
    //   }
    // }
    this.stompClient.unsubscribe('/topic/' + id)
  },
  ...
  send(data) {
    ...
    this.stompClient.send('/app/receive')
  },
